...
--
-- Хост: MySQL-8.4:3306
-- Время создания: Июн 01 2026 г., 03:04
...
--
-- Дублирующая структура для представления `admin_order_details`
...
INSERT INTO `orders` (`id`, `status_id`, `table_number`, `discount_type_id`, `total_price`, `price_without_discount`, `payment_method`, `updated_at`) VALUES
(1, 2, 1, 3, 410.00, 410.00, 'Картка', '2026-05-27 12:30:00'),
(2, 3, 5, 1, 960.50, 1130.00, NULL, '2026-05-27 14:15:00');
...
INSERT INTO `order_item` (`order_id`, `dish_id`, `quantity`, `price_at_purchase`) VALUES
(1, 1, 2, 150.00),
(1, 23, 2, 55.00),
(2, 2, 1, 650.00),
(2, 6, 1, 260.00),
...
CREATE TRIGGER `after_order_item_update` AFTER UPDATE ON `order_item` FOR EACH ROW BEGIN
    -- Пересчитываем сумму для заказа, к которому относится обновленная позиция
    UPDATE orders
    SET price_without_discount = (
        SELECT COALESCE(SUM(price_at_purchase * quantity), 0)
        FROM order_item
    ...
    -- Если вдруг у позиции поменялся сам order_id (что бывает редко, но для подстраховки нужно), 
    -- пересчитываем сумму и для старого заказа, из которого забрали позицию.
...
--
-- Дублирующая структура для представления `waiter_order_details`
-- (См. Ниже фактическое представление)
--
CREATE TABLE `waiter_order_details` (
`dish_name` varchar(30)
,`image_path` varchar(300)
,`order_id` bigint
,`price_at_purchase` decimal(10,2)
